...
PLAN_DEFINITIONS: list[dict[str, Any]] = [
    dict(slug=SubscriptionPlanSlug.STARTER, name="Starter", max_listings=1, price_eur_cents=800),
    dict(slug=SubscriptionPlanSlug.BASIC, name="Basic", max_listings=5, price_eur_cents=2500),
    dict(slug=SubscriptionPlanSlug.PRO, name="Pro", max_listings=10, price_eur_cents=4000),
    dict(slug=SubscriptionPlanSlug.BUSINESS, name="Business", max_listings=15, price_eur_cents=5500),
    dict(slug=SubscriptionPlanSlug.ENTERPRISE, name="Enterprise", max_listings=-1, price_eur_cents=0),
]
...
async def ensure_subscription_plans() -> None:
    ...
    for plan_def in PLAN_DEFINITIONS:
        ...
        if client is not None and slug != SubscriptionPlanSlug.ENTERPRISE:
            try:
                stripe_price_id = await asyncio.to_thread(_find_or_create_stripe_price, client, plan_def)
            except Exception:
